...
import { resolveBranchFromRequest, parseBranchId } from "./resolve-branch-from-request.ts";
...
// ===== 2ב-2: parseBranchId — אימות ערך ה-Cookie בצד-שרת (ל-initialBranch) =====
const VALID = [13, 14, 15, 16, 18, 19];

test("Cookie תקין '15' → 15", () => {
  assert.equal(parseBranchId("15", VALID), 15);
});

test("Cookie תקין '14' → 14", () => {
  assert.equal(parseBranchId("14", VALID), 14);
});

test("Cookie לא-חוקי '999' → null", () => {
  assert.equal(parseBranchId("999", VALID), null);
});

test("Cookie חסר (undefined) → null", () => {
  assert.equal(parseBranchId(undefined, VALID), null);
});

test("Cookie ריק '' → null", () => {
  assert.equal(parseBranchId("", VALID), null);
});

test("Cookie לא-מספרי 'abc' → null", () => {
  assert.equal(parseBranchId("abc", VALID), null);
});

test("Cookie לא-שלם '15.5' → null", () => {
  assert.equal(parseBranchId("15.5", VALID), null);
});
