...
export function ProductCard({
  ...
  return (
    <div
      ...
      className="group border border-line rounded-lg bg-white overflow-hidden flex flex-col cursor-pointer transition hover:shadow-[0_8px_22px_rgba(0,0,0,0.08)]"
    >
      {/* image */}
      <div className="relative aspect-square bg-white grid place-items-center p-4">
        {product.image && imgOk ? (
          ...
          <div className="w-16 h-20 rounded-lg bg-gradient-to-b from-cream to-gold-soft/40 grid place-items-center text-2xl text-gold">
            🛍️
          </div>
        )}
        {product.isKitchen && (
          <span className="absolute top-2.5 start-2.5 text-[10px] bg-gold-soft/80 text-wine font-bold rounded-full px-2 py-0.5">
      ...
      <div className="px-4 pb-4 pt-1 flex flex-col flex-1">
        ...
        <div className="text-[13px] text-ink/85 leading-snug mt-1 line-clamp-3 min-h-[3.2em]">
          {name}
        </div>
        ...
        <div className="mt-3 flex items-center justify-end">
          {qty > 0 ? (
            <div
              onClick={stop}
              className="flex items-center gap-3 border border-wine rounded-lg px-2.5 py-1 text-wine"
            >
